...
def fetch_team_roster(team_id: int, week_id: int) -> pd.DataFrame:
    ...
    try:
        ...
        for player in roster:
            ...
            is_pitcher = any(pos in ['SP', 'RP', 'P'] for pos in positions) or display_pos in ['SP', 'RP', 'P']
            
            if is_pitcher:
                pitchers.append({
                    'pitcher_name': player.full_name.strip(),
                    'pitcher_name_normalized': normalize_name(player.full_name.strip()),
...
def match_pitchers_to_starts(roster_df: pd.DataFrame, all_starts_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    matched = all_starts_df[all_starts_df['Pitcher_Normalized'].isin(roster_names)].copy()
...
def main():
    st.set_page_config(
        page_title="Yahoo Fantasy Pitcher Tracker",
        page_icon="⚾",
        layout="wide"
    )
    ...
    # Debug mode toggle
    debug_mode = st.sidebar.checkbox("🔍 Debug Mode", value=False, help="Show raw scraped data for troubleshooting")
    ...
    today = datetime.now()
    default_week = current_week + 1 if today.weekday() == 6 else current_week  # 6 = Sunday
    ...
    if st.session_state.get('data_loaded', False):
        ...
        with tab1:
            ...
            if debug_mode and not all_starts.empty:
            ...
            if not user_roster.empty:
                ...
                if not my_starts.empty:
                    ...
                    st.info("No probable starts found for your pitchers in the upcoming schedule.")
            else:
        ...
        with tab2:
            if opponent_id:
                opponent_team_name = st.session_state.get('opponent_team_name', f'Team {opponent_id}')
                st.header(f"🎯 {opponent_team_name} - Probable Starts (Week {week_id})")
                
                opponent_roster = st.session_state.get('opponent_roster', pd.DataFrame())
        ...
        with tab3:
            if opponent_id and not user_roster.empty:
                opponent_team_name = st.session_state.get('opponent_team_name', f'Team {opponent_id}')
                st.header(f"⚖️ Week {week_id} Matchup: {user_team_name} vs {opponent_team_name}")
                ...
                with col1:
                    ...
                    if not my_starts.empty:
                        ...
                        # Show starts
                        st.data_editor(
                            my_starts[['Date', 'Pitcher', 'Opponent']],
                            hide_index=True,
                            disabled=True,
                            use_container_width=True
                        )
                ...
                with col2:
                    ...
                    if not opp_starts.empty:
                        ...
                        # Show starts
                        st.data_editor(
                            opp_starts[['Date', 'Pitcher', 'Opponent']],
                            hide_index=True,
                            disabled=True,
                            use_container_width=True
                        )
                ...
                # Volume comparison
                st.subheader("Start Volume Advantage")
                diff = len(my_starts) - len(opp_starts)
                if diff > 0:
                    st.success(f"✅ You have {diff} more probable start(s) than your opponent")
                elif diff < 0:
                    st.error(f"⚠️ Your opponent has {abs(diff)} more probable start(s) than you")
                else:
                    st.info("⚖️ Both teams have the same number of probable starts")
            else:
